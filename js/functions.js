    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (leave_type == "Short") {
                ...
                from.value = new Date('1970-01-01T' + data[0] + 'Z')
                    .toLocaleTimeString('en-US',
                        { timeZone: 'UTC', hour12: true, hour: 'numeric', minute: 'numeric' }
                    );
                to.value = new Date('1970-01-01T' + data[1] + 'Z')
                    .toLocaleTimeString('en-US',
                        { timeZone: 'UTC', hour12: true, hour: 'numeric', minute: 'numeric' }
                    );
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data == "Approved") {
                ...
                approve_btn.parentElement.parentElement.children[7].innerHTML = "Approved";
            }
        }
    })
}
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data == "Rejected") {
                ...
                reject_btn.parentElement.parentElement.children[7].innerHTML = "Rejected";
            }
            ...
            if (data.length > 1) {
                for (i = 0; i < data.length; i++) {
                    ...
                    if (data[i].leave_type != "Full Day") {
                        ...
                        td2.innerHTML = data[i].duration.slice(0, 1) + " Hours, " + data[i].duration.slice(2, 4) + " Minutes";
                        tr.appendChild(td2);
                        let new_start_time = data[i].start_time;
                        let timeString12hr = new Date('1970-01-01T' + new_start_time + 'Z')
                            .toLocaleTimeString('en-US',
                                { timeZone: 'UTC', hour12: true, hour: 'numeric', minute: 'numeric' }
                            );
                        let new_end_time = data[i].end_time;

                        let timeString12hr1 = new Date('1970-01-01T' + new_end_time + 'Z')
                            .toLocaleTimeString('en-US',
                                { timeZone: 'UTC', hour12: true, hour: 'numeric', minute: 'numeric' }
                            );
...
function view_attendance(id) {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            time_in.innerHTML = (data[0].time_in).slice(0, 5) + " " + (data[0].time_in).slice(8);
            time_out.innerHTML = (data[0].time_out).slice(0, 5) + " " + (data[0].time_out).slice(8);
            $.ajax({
                type: "Post",
                url: "include/functions.php",
                data: {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data == "Approved") {
                ...
                approve_btn.parentElement.parentElement.children[5].innerHTML = "Approved";
            }
        }
    })
}
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data == "Rejected") {
                ...
                reject_btn.parentElement.parentElement.children[5].innerHTML = "Rejected";
            }
...
function user_attendance(user_id, user_name, department, designation, user_status) {
    let tbody = document.getElementById("t2");
    tbody.innerHTML = "";
    let tr = document.createElement("tr");
    let td1 = document.createElement("td");
        ...
        success: function (data) {
            ...
            if (data[0] != "No leaves applied") {
                for (let i = 0; i < data.length; i++) {
                    ...
                    if (data[i].leave_type != "Full Day") {
                        let td2 = document.createElement("td");
                        td2.innerHTML = data[i].duration.slice(0, 1) + " Hours, " + data[i]
                            .duration.slice(2, 4) + " Minutes";
                        tr.appendChild(td2);
                        let new_start_time = data[i].start_time;
                        let timeString12hr = new Date('1970-01-01T' + new_start_time + 'Z')
                            .toLocaleTimeString('en-US', {
        ...
        success: function (data) {
            ...
            if (data[1] != "No data found") {
                for (i = 1; i < data.length; i++) {
                    ...
                    a.classList.add("dropdown-item", "d-flex", "align-items-center", "m-0", "p-0");
                    a.href = "#";
                    a.id = data[i].a_id;
                    a.setAttribute("onclick", "alert_modal(this.id); change_status(this.id);");
